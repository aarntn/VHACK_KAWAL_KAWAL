...
4. Set env vars:
   ...
   - `FRAUD_ENGINE_URL=https://<fraud-service>.onrender.com/score_transaction` **(required override; do not keep Dockerfile local-compose default)**
   - `FRAUD_ENGINE_HEALTH_URL=https://<fraud-service>.onrender.com/health` **(required override; do not keep Dockerfile local-compose default)**
5. Add a health check path: `/health/ready`.
...
Model artifacts are baked into the fraud image at build time from the promoted manifest and referenced files:
...
At startup, the fraud API refuses traffic unless:

- the manifest file exists,
- each manifest-referenced artifact file exists, and
- each artifact SHA-256 matches the digest pinned in the manifest.
...
# Artifact manifest (authoritative)
PROMOTED_ARTIFACT_MANIFEST_FILE=/app/project/models/promoted_artifact_manifest.json

# Legacy artifact vars are deprecated/ignored when manifest loading is enabled:
# FRAUD_MODEL_FILE=...
# FRAUD_FEATURE_FILE=...
