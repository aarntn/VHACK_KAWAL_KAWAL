...
def run_suite(approve_margin: float, block_margin: float) -> dict[str, Any]:
    ...
    large_score = suite["presets"]["large_amount"]["final_risk_score"]
    cross_score = suite["presets"]["cross_border"]["final_risk_score"]

    checks = {
        "everyday_approve_buffer": (
            suite["presets"]["everyday_purchase"]["fraud_decision"] == "APPROVE"
            and everyday_score < float(approve_threshold) - approve_margin
        ),
        "large_flag_band": (
            suite["presets"]["large_amount"]["fraud_decision"] == "FLAG"
            and float(approve_threshold) <= large_score < float(block_threshold)
        ),
        "cross_border_block_band_or_rule_hit": (
        ...
        "wallet_flag_pending_verification": (
            suite["presets"]["large_amount"]["wallet_decision"] == "FLAG"
            and suite["presets"]["large_amount"]["wallet_action"] == "PENDING_VERIFICATION"
        ),
        "wallet_block_decline": (
            suite["presets"]["cross_border"]["wallet_decision"] == "BLOCK"
