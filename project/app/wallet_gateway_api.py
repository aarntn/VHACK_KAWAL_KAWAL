...
from .schemas import ScoreTransactionRequest, WalletAuthorizeRequest, WalletAuthorizeResponse
...
UPSTREAM_TIMEOUT_SECONDS = float(os.getenv("UPSTREAM_TIMEOUT_SECONDS", "0.8"))

UPSTREAM_MAX_RETRIES = int(os.getenv("UPSTREAM_MAX_RETRIES", "1"))
UPSTREAM_BACKOFF_MS = float(os.getenv("UPSTREAM_BACKOFF_MS", "20"))
UPSTREAM_BACKOFF_MAX_MS = float(os.getenv("UPSTREAM_BACKOFF_MAX_MS", "150"))
...
WALLET_CORS_ALLOW_ORIGINS = os.getenv("WALLET_CORS_ALLOW_ORIGINS", "").strip()
...
def parse_allowed_cors_origins(raw_value: str) -> list[str]:
    ...
    if "*" in parsed:
        logger.warning(
            "Wildcard CORS origin requested for wallet gateway; ignoring and falling back to localhost-only defaults"
        )
        return list(DEFAULT_BROWSER_ORIGINS)
    return parsed or list(DEFAULT_BROWSER_ORIGINS)


def _safe_upstream_target(url: str) -> str:
    parsed = urlsplit(url)
...
async def lifespan(_: FastAPI):
    ...
    with _runtime_init_lock:
        if UPSTREAM_HTTP is None:
            ...
            _runtime_init_count += 1
        logger.info(
            "startup_upstream_targets fraud_engine=%s fraud_engine_health=%s",
            _safe_upstream_target(FRAUD_ENGINE_URL),
            _safe_upstream_target(FRAUD_ENGINE_HEALTH_URL),
        )
        logger.info(
...
ALLOWED_CORS_ORIGINS = parse_allowed_cors_origins(WALLET_CORS_ALLOW_ORIGINS)
...
async def _attempt_fraud_call(fraud_payload: dict) -> tuple[dict | None, dict]:
    ...
    for attempt in range(max_attempts):
        ...
        try:
            ...
            attempt_dur = round((time.perf_counter() - attempt_start) * 1000, 3)
            attempt_durations_ms.append(attempt_dur)
            ...
            return None, {
                ...
                "backoff_total_ms": round(backoff_total_ms, 3),
                "upstream_call_ms": None,
            }
...
def build_fallback_fraud_result(error_code: str) -> dict:
    return {
        ...
        "corridor": None,
        "normalized_amount_reference": None,
        "normalization_basis": "wallet_fallback_without_upstream_score",
    }
...
def build_fraud_payload(payment: "WalletPaymentRequest") -> dict:
    ...
    return {
        ...
        for field_name in ScoreTransactionRequest.model_fields
    }
...
async def authorize_payment(payment: WalletPaymentRequest, request: Request):
    ...
    try:
        ...
        runtime_mode = str(fraud_result.get("runtime_mode", "primary"))
        final_risk_score = fraud_result["final_risk_score"]
        fraud_reasons = fraud_result["reasons"]
        reason_codes = list(fraud_result.get("reason_codes", []))
        corridor = fraud_result.get("corridor")
        normalized_amount_reference = fraud_result.get("normalized_amount_reference")
        normalization_basis = fraud_result.get("normalization_basis")
            ...
            json.dumps(
                {
                    ...
                    "upstream_call_ms": upstream_meta.get("upstream_call_ms"),
                    "attempt_durations_ms": upstream_meta.get("attempt_durations_ms", []),
                    "backoff_total_ms": upstream_meta.get("backoff_total_ms", 0.0),
        ...
        return WalletAuthorizeResponse(
            ...
            normalized_amount_reference=float(normalized_amount_reference) if normalized_amount_reference is not None else None,
            normalization_basis=str(normalization_basis) if normalization_basis is not None else None,
            explainability=None,
            upstream_attempts=int(upstream_meta["attempts"]),
