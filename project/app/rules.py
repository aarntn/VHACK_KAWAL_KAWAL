...
def infer_geo_bucket(tx: Dict[str, Any]) -> str:
    source_country = str(tx.get("source_country", "") or "").strip().upper()
    destination_country = str(tx.get("destination_country", "") or "").strip().upper()
    if source_country and destination_country:
        if source_country == destination_country:
            return f"DOMESTIC:{source_country}"
        return f"CORRIDOR:{source_country}-{destination_country}"
    if source_country:
        return f"COUNTRY:{source_country}"
    if destination_country:
        return f"COUNTRY:{destination_country}"
