...
interface Scenario {
  ...
  crossBorder?: boolean;
  prefillSourceCountry?: CountryCode;
  prefillDestinationCountry?: CountryCode;
  prefillConnectivityMode?: ConnectivityMode;
  prefillAgentAssisted?: boolean;
}
...
const SCENARIOS: Scenario[] = SCENARIO_ORDER.map((scenarioId) => {
  ...
  return {
    ...
    prefillSourceCountry: preset.ui.source_country,
    prefillDestinationCountry: preset.ui.destination_country,
    prefillConnectivityMode: preset.ui.connectivity_mode,
    prefillAgentAssisted: preset.ui.is_agent_assisted,
  };
});
...
const SCENARIO_I18N_KEY_BY_ID: Record<string, string> = {
  ...
  agent_cash_out: "scenario.agent_cash_out",
...
export default function CheckYourPayment({
  ...
  const [userId, setUserId] = useState("user_id_qr_local");
  const [amount, setAmount] = useState("185000");
  const [paymentType, setPaymentType] = useState("MERCHANT");
  const [crossBorder, setCrossBorder] = useState(false);
  const [walletId, setWalletId] = useState("wallet_id_qr_local");
  const [currency, setCurrency] = useState("IDR");
  const [merchant, setMerchant] = useState("Warung Nusantara QR");
  const [sourceCountry, setSourceCountry] = useState<CountryCode>("ID");
  const [destinationCountry, setDestinationCountry] = useState<CountryCode>("ID");
  const [isAgentAssisted, setIsAgentAssisted] = useState(false);
  const [connectivityMode, setConnectivityMode] = useState<ConnectivityMode>("online");
  const [deviceRiskScore, setDeviceRiskScore] = useState("0.04");
  const [ipRiskScore, setIpRiskScore] = useState("0.05");
  const [locationRiskScore, setLocationRiskScore] = useState("0.04");
  const [deviceId, setDeviceId] = useState("device_id_qr_safe_01");
  const [deviceSharedUsers24h, setDeviceSharedUsers24h] = useState("1");
  const [accountAgeDays, setAccountAgeDays] = useState("420");
  const [simChangeRecent, setSimChangeRecent] = useState(false);
  const [channel, setChannel] = useState<"APP" | "WEB" | "AGENT" | "QR">("QR");
  ...
  function handleScenarioSelect(scenario: Scenario) {
    ...
    if (scenario.id === "custom") {
      ...
      setMerchant("");
      setSourceCountry("SG");
      setDestinationCountry("SG");
      setIsAgentAssisted(false);
    ...
    if (scenario.prefillSourceCountry) setSourceCountry(scenario.prefillSourceCountry);
    if (scenario.prefillDestinationCountry) setDestinationCountry(scenario.prefillDestinationCountry);
    if (scenario.prefillConnectivityMode) setConnectivityMode(scenario.prefillConnectivityMode);
    setIsAgentAssisted(Boolean(scenario.prefillAgentAssisted));
  ...
  function handleSubmit() {
    ...
    const payload: UserFormPayload = {
      ...
      destination_country: (crossBorder ? destinationCountry : sourceCountry),
      is_agent_assisted: isAgentAssisted,
      connectivity_mode: connectivityMode,
      preset_risk:
      ...
      <div style={{ flex: 1, padding: "8px 16px 140px 16px", overflowY: "auto" }}>
        ...
        <SectionCard title={translate("form.section.paymentDetails")}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
            <div>
              <FieldLabel>Asean source country</FieldLabel>
              <select
                value={sourceCountry}
                onChange={(e) => setSourceCountry(e.target.value as CountryCode)}
            ...
            <div>
              <FieldLabel>Asean destination country</FieldLabel>
              <select
                value={crossBorder ? destinationCountry : sourceCountry}
                onChange={(e) => setDestinationCountry(e.target.value as CountryCode)}
                disabled={!crossBorder}
                style={{ width: "100%", background: tokens.bgInput, border: `0.9px solid ${tokens.border}`, borderRadius: 8, padding: "11.5px 12px", fontSize: 12, color: tokens.textPrimary, opacity: crossBorder ? 1 : 0.7 }}
              >
                <option value="SG">SG</option>
                <option value="MY">MY</option>
                <option value="ID">ID</option>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 12 }}>
            <div>
              <FieldLabel>Connectivity mode</FieldLabel>
              <select
                value={connectivityMode}
                onChange={(e) => setConnectivityMode(e.target.value as ConnectivityMode)}
                style={{ width: "100%", background: tokens.bgInput, border: `0.9px solid ${tokens.border}`, borderRadius: 8, padding: "11.5px 12px", fontSize: 12, color: tokens.textPrimary }}
              >
                <option value="online">Live</option>
                <option value="intermittent">Intermittent</option>
                <option value="offline_buffered">Degraded</option>
              </select>
            </div>
            <button
              onClick={() => setIsAgentAssisted((prev) => !prev)}
            ...
            >
              <input type="checkbox" checked={isAgentAssisted} readOnly />
              Agent-assisted
            </button>
          </div>
        </SectionCard>
