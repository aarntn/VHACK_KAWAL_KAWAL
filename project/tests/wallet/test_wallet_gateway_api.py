...
class WalletGatewayApiTests(unittest.TestCase):
    ...
    def test_wallet_forwards_asean_context_fields_to_fraud_engine(self) -> None:
        ...
        def mocked_post(url, json, headers=None):
            ...
            return MockFraudResponse(
                ...
                payload={
                    ...
                    "reason_codes": ["ASEAN_AGENT_ASSISTED_CASH_OUT"],
                    "corridor": "MY-MY",
                    "normalized_amount_reference": 42.5,
                    "normalization_basis": "asean_currency_normalization_v1",
                },
            )
        ...
        self.assertEqual(captured_payload.get("currency"), "MYR")
        self.assertEqual(captured_payload.get("source_country"), "MY")
        self.assertEqual(captured_payload.get("destination_country"), "MY")
        self.assertTrue(captured_payload.get("is_agent_assisted"))
        self.assertEqual(captured_payload.get("connectivity_mode"), "intermittent")
        self.assertIsNotNone(captured_payload.get("forwarded_correlation_id"))
        body = response.json()
        self.assertEqual(body["wallet_action"], "PENDING_VERIFICATION")
        self.assertEqual(body["runtime_mode"], "primary")
        self.assertEqual(body["corridor"], "MY-MY")
        self.assertEqual(body["normalized_amount_reference"], 42.5)
        self.assertEqual(body["normalization_basis"], "asean_currency_normalization_v1")
    ...
    def test_zz_env_overrides_update_upstream_urls_and_safe_target_logging(self) -> None:
        original_score_url = wallet_gateway_api.FRAUD_ENGINE_URL
        original_health_url = wallet_gateway_api.FRAUD_ENGINE_HEALTH_URL
        override_score_url = "https://fraud-staging.onrender.com/score_transaction"
        override_health_url = "https://fraud-staging.onrender.com/health"

        try:
            os.environ["FRAUD_ENGINE_URL"] = override_score_url
            os.environ["FRAUD_ENGINE_HEALTH_URL"] = override_health_url
            importlib.reload(wallet_gateway_api)

            self.assertEqual(wallet_gateway_api.FRAUD_ENGINE_URL, override_score_url)
            self.assertEqual(wallet_gateway_api.FRAUD_ENGINE_HEALTH_URL, override_health_url)
            self.assertEqual(
                wallet_gateway_api._safe_upstream_target(wallet_gateway_api.FRAUD_ENGINE_URL),
                "https://fraud-staging.onrender.com",
            )
            self.assertEqual(
                wallet_gateway_api._safe_upstream_target(wallet_gateway_api.FRAUD_ENGINE_HEALTH_URL),
                "https://fraud-staging.onrender.com",
            )
        finally:
            os.environ["FRAUD_ENGINE_URL"] = original_score_url
            os.environ["FRAUD_ENGINE_HEALTH_URL"] = original_health_url
            importlib.reload(wallet_gateway_api)
